...
for EXP_NUM in range(1): #[33, 34, 35, 40, 41] BOTH THESE RUNNING NOW. old machine running 49, 40, 41 in tandem
    for unlabeled_data in [5000]:
        ...
        L1_weight=1e-3
        THRESH_HOLD_VAL=.01
        layers=[4096,2048,1024]#[500,500,500,300,300,400,800]#[3000,1000,500,300,300,300,300,300,300,300,300,300,300,300]#[3000,1500,1000,500,500,500,300,300,300,400,400,800]#[1000,1000,1000,500,500,500,300,300,300,400,400,400,800
        ...
        if Track_run:
            ...
            wandb.login(key=key) 
            wandb.init(project='rtt unlabeled', name=f'lowcoh perturbed grid 3 layer 5k data no scaling anywhere')
        ...
        indim=(training_data.b[0].shape[0]/2)
        ...
        for epoch in range(Epochs):
            ...
            for batch, (b, rho, num_targets) in enumerate(trainloader):
                if True:
                    ...
                    if GELMA>0 and epoch>0:
                        ...
                        #Error=F.normalize((b_hat-b).squeeze(), dim=-1).squeeze()
                        ...
                        #GELMA_inners=(torch.inner(Error,GELMA_out.squeeze()).diagonal(dim1=-2, dim2=-1))
                        #GELMA_inners=sum(GELMA_inners)/(torch.numel(GELMA_inners))
                        ...
                        #GELMA_inners=(torch.inner(Error,GELMA_out.squeeze()).diagonal(dim1=-2, dim2=-1))
                        #GELMA_inners=GELMA*sum(GELMA_inner  s)/(torch.numel(GELMA_inners))
                        GELMA_inners_coupled.backward(retain_graph=True)
            ...
            if epoch%1==0:
                ...
                wand_dict['num outliers']=len(index_list_outliers)
            if Track_run:
                wandb.log(wand_dict)
