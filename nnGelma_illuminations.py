...
for unlabeled_data in [5000]: 
    for betas in [(.95, .444)]:#,[os.path.join(cwd,'Data/PNAS-regime_all_seeds/PNAS-regime_seed0'), os.path.join(cwd,'Data/FoldyLox_all_seeds/FoldyLox_seed0'),  os.path.join(cwd,'Data/PNAS-lowcoh_regime_all_seeds/PNAS-lowcoh_regime_seed0'), os.path.join(cwd,'Data/PNAS-highcoh_regime_all_seeds/PNAS-highcoh_regime_seed0')]:
        for EXP_NUM in range(1): #NEED 40-50
            SCALING_param='None'
            WDECAY=1e-2
            ...
            if Track_run:
                ...
                wandb.init(project='rtt unlabeled', name=f'data type={data_type}, betas={betas}, Optimal params for seed0. GELMA lr not rescaled.')
                ...
                wandb.config['dcitonary size']=DICTIONARY_size
            ...
            if DICTIONARY_size=='default':
                ...
                enc_out=DICTIONARY_size
                dec_in=DICTIONARY_size
            if encoder_type=='fc':
            ...
            if DICTIONARY_size=='default':
                ...
                Complex_eye=torch.cat((torch.eye(int(DICTIONARY_size)), torch.zeros((int(DICTIONARY_size),int(DICTIONARY_size)))), dim=1).to(device)
            ...
            #  L2_loss=L2_loss_fn(F.normalize(b_hat.squeeze(), dim=-1), F.normalize(b_val.squeeze()))
                ...
                if epoch%1==0:
                    ...
                    if epoch%1000==0:
                        ...
                        torch.save(encoder.state_dict(), f'/home/achristie/Codes_data/Experiment_data/rhosupport_stats/Scaling_tests/10kepochs/enocder_{unlabeled_data}_{EXP_NUM}_{timestampStr}.pt')
                        torch.save(decoder.state_dict(), f'/home/achristie/Codes_data/Experiment_data/rhosupport_stats/Scaling_tests/10kepochs/decoder_{unlabeled_data}_{EXP_NUM}_{timestampStr}.pt')
                        torch.save(GELMA_net.state_dict(), f'/home/achristie/Codes_data/Experiment_data/rhosupport_stats/Scaling_tests/10kepochs/GELMA_{unlabeled_data}_{EXP_NUM}_{timestampStr}.pt')
                    if epoch==4999:
                        torch.save(encoder.state_dict(), f'/home/achristie/Codes_data/Experiment_data/rhosupport_stats/Scaling_tests/5kepochs/enocder_{unlabeled_data}_{EXP_NUM}_{timestampStr}.pt')
                        torch.save(decoder.state_dict(), f'/home/achristie/Codes_data/Experiment_data/rhosupport_stats/Scaling_tests/5kepochs/decoder_{unlabeled_data}_{EXP_NUM}_{timestampStr}.pt')
                        torch.save(GELMA_net.state_dict(), f'/home/achristie/Codes_data/Experiment_data/rhosupport_stats/Scaling_tests/5kepochs/GELMA_{unlabeled_data}_{EXP_NUM}_{timestampStr}.pt')
