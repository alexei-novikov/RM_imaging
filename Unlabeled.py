...
layers_list=[[2**i]*10 for i in [9]]

Epochs=30000

unlabeled_data_list=[5000]

for lin_type in ['complex']:
    for GELMA, L1_weight in [(0, 0)]:
        for acti in ['mod_relu']:
            for unlabeled_data in unlabeled_data_list:
                for layers in layers_list:
                    for data_path in data_path_list:
                        ...
                        encoder=M.fc_net_extra(enc_dim, layers,outdim, net_type='fc',linear_type=lin_type, activation=acti, bias=True, out_scaling=None,dropout=.5)
                        ...
                        if Track_run:
                            ...
                            wandb.config['run']=CE
                            wandb.config['time stamp']=timestampStr
                        wand_dict={}
